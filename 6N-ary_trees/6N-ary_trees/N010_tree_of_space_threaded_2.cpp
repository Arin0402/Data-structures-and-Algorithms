...
public:
    ...
    {
        TreeNode *node = mp[s];
        if (node->isResourceInUse)
            while (node->isResourceInUse)
                ;
        node->isResourceInUse = false;

        if (node->isLocked || !node->isLockable)
        {
            node->isResourceInUse = false;
            return false;
        }
    ...
    bool unlock(string s, int uid)
    {
        ...
        if (node->isResourceInUse)
            while (node->isResourceInUse)
                ;
        node->isResourceInUse = true;
        if (!node->isLocked || (node->isLocked && node->uid != uid))
        {
            node->isResourceInUse = false;
            return false;
        }
    ...
    void unblockResource(TreeNode *n)
    {
        queue<TreeNode *> q;
        q.push(n);
        while (!q.empty())
